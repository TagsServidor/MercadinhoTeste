...
$sql = "SELECT * FROM condominios where condominio_central = '$_POST[central]'   ORDER BY condominio_nome";
...
<script>
    ...
    $('#unidades').on('change', function() {
		...
		$.ajax({
			url: 'listar_unidades_lista.php?central=<?php echo $_POST['central'] ?>',
			cache: false,
			data: dados,
			type: "POST",  
